...
def cdo_write( node, sdt, outfile, verbose=0 ):
    ...
    #
    # pass #1: gather up the nodes
    #
    node = fdt.path_offset( "/" )
    depth = 0
    while depth >= 0:
    ...
    for depth, nodeoffset, nodename in node_list:
        node_type = Lopper.node_type( fdt, nodeoffset )
        ...
        if re.match( "^clock-controller$", cdo_t ):
            ...
            for c in sub_clocks:
                cn,cns = Lopper.node_find_by_name( fdt, c, 0 )
                if cn > 0:
    ...
    for depth, nodeoffset, nodename in node_list:
        ...
        if not node_type:
            ...
            if depth > 1:
                print( "# [DBG]+: trying to infer type from parent node (depth: %s)"  % depth, file=output )
                # try and infer the type from the parents type
                ptype = parent[depth-1][1]
                if ptype:
                    print("# parent type: %s" % ptype, file=output )
                    node_type = ptype

        nodeid = cdo_nodeid( nodeoffset, str(node_type) )
        parent[depth] = [ nodeid, node_type, nodename ]

        if node_type:
            print( "# node start: [%s]:%s depth: %s offset: %s type: %s (%s)" %
                   (hex(nodeid), nodename, depth, hex(nodeoffset), str(node_type), type(node_type) ) , file=output)
            ...
            if re.match( "root", cdo_t ):
                ...
                print( "pm_add_node {0} {1} {2}".format( hex(nodeid), pm_add_word_one, hex(parent[depth-1][0])), file=output)
                ...
                print( "# cpu cluster, depth: {0}".format(depth), file=output)
                print( "pm_add_node {0} {1} {2}".format( hex(nodeid), pm_add_word_one, hex(parent[depth-1][0])), file=output )
                ...
                ref_count = sdt.node_ref( nodename )
                if ref_count < 0:
                ...
                for c in sub_clocks:
                    ...
                    if cn > 0:
                        ...
                        sub_nodeid = cdo_nodeid( cn, sub_node_type )

                        sub_clocktype = Lopper.node_type( fdt, cn )
                        sub_clock_mask = 0x0
                ...
                pm_add_word_one = cdo_parent_word( parent_count, width, shift )
                print( "# unknown type, depth: {0}".format(depth), file=output)
                print( "pm_add_node {0} <flags> {1}".format( hex(nodeid), hex(parent[depth-1][0])), file=output )

            print( "# node end: [%s]:%s" % (hex(nodeid), nodename) , file=output )
        else:
            # TODO: what do to about these ? For now, we just log them
            print( "# INFO: skipping node with no type: %s (depth %s)" % (nodename,depth), file=output)
            print( "#       parent was: %s" % parent[depth-1], file=output )

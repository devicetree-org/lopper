...
from gen_domain_dts import (
    delete_unused_props,
    xlnx_generate_domain_dts,
    _node_must_be_preserved,
)
...
def _xlnx_zephyr_fixup_zynqmp_ipi_child(remote_node, sdt, schema):
    """Promote raw SDT IPI mailbox children or drop invalid ones.

    VCK190/Versal SDT nodes may use ``child@N`` placeholders with ``xlnx,ipi-*``
    buffer metadata but no ``reg``. Zephyr's ``xlnx,zynqmp-ipi-mailbox`` child-binding
    requires ``reg``, ``reg-names``, and ``remote-ipi-id`` (not ``compatible`` on
    children — see upstream 04eb83c).

    For ZynqMP boards with fully-formed ``ipi@`` children (e.g. ZCU102), prune to
    the child-binding properties only; do not re-add ``compatible``.
    """
    compat = remote_node.propval("compatible", list)
    reg = remote_node.propval("reg", list)
    has_compat = compat and compat != ['']
    has_reg = reg and reg != [''] and len(reg) > 1

    if not has_compat and not has_reg:
        req = remote_node.propval("xlnx,ipi-req-msg-buf", list)
        rsp = remote_node.propval("xlnx,ipi-rsp-msg-buf", list)
        if (not req or req == ['']) or (not rsp or rsp == ['']):
            sdt.tree.delete(remote_node)
            return

        remote_node + LopperProp(name="compatible", value=["xlnx,zynqmp-ipi-dest-mailbox"])
        remote_node + LopperProp(
            name="reg",
            value=[0, req[0], 0, 0x20, 0, rsp[0], 0, 0x20],
        )
        remote_node + LopperProp(
            name="reg-names",
            value=["local_request_region", "local_response_region"],
        )

    _xlnx_zephyr_convert_zynqmp_ipi_id(remote_node)

    child_schema = [
        entry for compatible, entry in schema.items()
        if compatible in remote_node.propval("compatible", list)
    ]
    if child_schema:
    ...
    reg = remote_node.propval("reg", list)
    if reg and reg != [''] and len(reg) > 1:
        sdt.tree.rename(remote_node, "ipi@%x" % reg[1])
        return

    if (not remote_node.propval("remote-ipi-id", list)
            or remote_node.propval("remote-ipi-id", list) == ['']):
        sdt.tree.delete(remote_node)
            ...
            if node.parent:
                if node.propval("compatible") != ['']:
                    ...
                    if "xlnx,zynqmp-ipi-mailbox" in node["compatible"].value:
                        ...
                        for remote_node in list(node.subnodes(children_only=True)):
                            _xlnx_zephyr_fixup_zynqmp_ipi_child(remote_node, sdt, schema)
                    # PS-IIC
                    ...
                    if is_supported_periph:
                        ...
                        if (node.name not in ("axi", "soc", "amba_pl") and
                                ...
                                not _node_must_be_preserved(node, sdt.tree)):
                            sdt.tree.delete(node)

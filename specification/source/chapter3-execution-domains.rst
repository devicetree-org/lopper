...
.. table:: Execution Domain v1 Properties
   ...
   ``access-implicit-default``         SD    ``<u32>``             See :numref:`domains-implicit-flags`
   ``#memory-implicit-default-cells``  SD    ``<u32>``             See :numref:`domains-implicit-flags`
   ``memory-implicit-default``         SD    ``<u32>``             See :numref:`domains-implicit-flags`
   ``#sram-implicit-default-cells``    SD    ``<u32>``             See :numref:`domains-implicit-flags`
   ``sram-implicit-default``           SD    ``<u32>``             See :numref:`domains-implicit-flags`
...
.. table:: ``cpus`` Property
   ...
   Description Defines the physical CPUs this domain (the domain in which
               the *cpus* property appears) runs on.
...
- ``baremetal`` refers to a direct application that executes on the system
  ...
  first stage boot loader, a second stage boot loader, U-Boot [U-Boot],
  Trusted Firmware-A [TF-A], etc.

- ``linux`` refers to a Linux based operating system. Examples of this may
  include Yocto Project [Yocto] derived distributions, Red Hat
  Enterprise Linux [RHEL], Ubuntu [Ubuntu] distributions, etc.

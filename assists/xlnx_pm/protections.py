...
SKIP_MODULES = [
    ...
    "versal_cips_0_pspmc_0", "pspmc@fffc0000",
        ...
        for sub_id, entries in ftb_ents.items():
            for entry in entries:   # ftb.FirewallTableEntry instance per subsystem
                ...
                for mid_entry in entry.mid_list:
                    mask_idx = x_inst.get_master_by_smrw(mid_entry.smid, mid_entry.mask, entry.rw)
    ...
    def generate_aper_masks(self, custom=0):
        ...
        for module in mod_list:
            ...
            if x_inst is not None:
                if debug > 0:
                    print("[DBG++] Aper Config for {} -> {}".format(module, x_inst_name))
                self.modules[module].compute_aper_mask(x_inst, custom)
            else:
                if debug > 0:
                    print("[DBG++] Skipped: Aper Config for {} -> {}".format(module, x_inst_name))
...
def setup_dev_ftb_entry(subsystem_id: int,
                        pm_name: str,
                        rw: int,
                        tz: int,
...
def ftb_setup_modules():
    for tline in firewall_table.tokens:
        ...
        mod_inst.add_ftb_entry(subsystem_id,
                               mod_inst.name,
                               baddr,
